...
function Profile({ isOpen, onClose, onRecipeDeleted }) {
    ...
    useEffect(() => {
        const fetchBadges = async () => {
            try {
                ...
                const response = await axios.get(`${process.env.REACT_APP_APP_URL}/api/cooking-badges`, {
    ...
    const fetchMyRecipes = async () => {
        try {
            const token = localStorage.getItem('token');
            const response = await axios.get(`${process.env.REACT_APP_APP_URL}/api/my-recipes`, {
                headers: { Authorization: `Bearer ${token}` }
            });
    ...
    const fetchSavedRecipes = async () => {
        try {
            const token = localStorage.getItem('token');
            const response = await axios.get(`${process.env.REACT_APP_APP_URL}/api/saved-recipes`, {
                headers: { Authorization: `Bearer ${token}` }
            });
    ...
    const handleImageChange = async (e) => {
        ...
        try {
            ...
            const response = await axios.put(
                `${process.env.REACT_APP_APP_URL}/api/profile`,
                formData,
                {
                    headers: {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            await axios.put(`${process.env.REACT_APP_APP_URL}/api/profile`, formData, {
                headers: {
                    Authorization: `Bearer ${token}`,
                    'Content-Type': 'multipart/form-data'
                }
    ...
    const handleDeleteRecipe = async (recipeId, event) => {
        ...
        if (window.confirm('chef, are you sure you want to delete this recipe?')) {
            try {
                const token = localStorage.getItem('token');
                await axios.delete(`${process.env.REACT_APP_APP_URL}/api/recipes/${recipeId}`, {
                    headers: { Authorization: `Bearer ${token}` }
                });
    ...
    const handleUnsaveRecipe = async (recipeId, event) => {
        ...
        try {
            const token = localStorage.getItem('token');
            await axios.post(
                `${process.env.REACT_APP_APP_URL}/api/recipes/${recipeId}/save`,
                {},
                {
                    headers: { Authorization: `Bearer ${token}` }
                }
    ...
    const handleRemoveProfilePicture = async () => {
        try {
            const token = localStorage.getItem('token');
            await axios.delete(`${process.env.REACT_APP_APP_URL}/api/profile/image`, {
                headers: { Authorization: `Bearer ${token}` }
            });
