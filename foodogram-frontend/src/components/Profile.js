...
function Profile({ isOpen, onClose, onRecipeDeleted }) {
    ...
    useEffect(() => {
        const fetchBadges = async () => {
            try {
                ...
                const response = await axios.get(`${window.location.origin}/api/cooking-badges`, {
    ...
    const fetchProfile = async () => {
        try {
            ...
            const response = await axios.get(`${window.location.origin}/api/users/${username}`, {
                headers: { Authorization: `Bearer ${token}` }
            });
    ...
    const fetchMyRecipes = async () => {
        try {
            const token = localStorage.getItem('token');
            const response = await axios.get(`${window.location.origin}/api/my-recipes`, {
                headers: { Authorization: `Bearer ${token}` }
            });
    ...
    const fetchSavedRecipes = async () => {
        try {
            const token = localStorage.getItem('token');
            const response = await axios.get(`${window.location.origin}/api/saved-recipes`, {
                headers: { Authorization: `Bearer ${token}` }
            });
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            await axios.put(`${window.location.origin}/api/profile`, formData, {
                headers: {
                    Authorization: `Bearer ${token}`,
                    'Content-Type': 'multipart/form-data'
                }
    ...
    const handleDeleteRecipe = async (recipeId, event) => {
        ...
        if (window.confirm('chef, are you sure you want to delete this recipe?')) {
            try {
                const token = localStorage.getItem('token');
                await axios.delete(`${window.location.origin}/api/recipes/${recipeId}`, {
                    headers: { Authorization: `Bearer ${token}` }
                });
    ...
    const handleUnsaveRecipe = async (recipeId, event) => {
        ...
        try {
            const token = localStorage.getItem('token');
            await axios.post(
                `${window.location.origin}/api/recipes/${recipeId}/save`,
                {},
                {
                    headers: { Authorization: `Bearer ${token}` }
                }
    ...
    const handleRemoveProfilePicture = async () => {
        try {
            const token = localStorage.getItem('token');
            await axios.delete(`${window.location.origin}/api/profile/image`, {
                headers: { Authorization: `Bearer ${token}` }
            });
