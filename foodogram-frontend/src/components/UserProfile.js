...
function UserProfile({ username, isOpen, onClose }) {
  ...
  const fetchUserRecipes = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await axios.get(`${window.location.origin}/api/recipes/user/${username}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleFollowToggle = async () => {
    try {
      ...
      if (newFollowingState) {
        await axios.post(`${window.location.origin}/api/users/${username}/follow`, {}, {
          headers: { Authorization: `Bearer ${token}` }
        });
      } else {
        await axios.delete(`${window.location.origin}/api/users/${username}/follow`, {
          headers: { Authorization: `Bearer ${token}` }
        });
      }
