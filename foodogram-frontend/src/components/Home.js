...
function Home() {
  ...
  const refreshToken = async () => {
    try {
      ...
      const response = await axios.post('http://51.20.74.171/api/refresh-token', {
        token: oldToken
      });
  ...
  const fetchNotifications = useCallback(async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await axios.get('http://51.20.74.171/api/notifications', {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleLike = async (recipeId) => {
    try {
      const token = localStorage.getItem('token');
      await axios.post(`http://51.20.74.171/api/recipes/${recipeId}/like`, {}, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleNotificationClick = async (notification) => {
    try {
      const token = localStorage.getItem('token');
      await axios.put(
        `http://51.20.74.171/api/notifications/${notification.id}/read`,
        {},
        {
          headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleMarkAllRead = async () => {
    try {
      const token = localStorage.getItem('token');
      await axios.put(
        'http://51.20.74.171/api/notifications/mark-all-read',
        {},
        {
          headers: { Authorization: `Bearer ${token}` }
        }
