...
function Home() {
  ...
  const refreshToken = async () => {
    try {
      ...
      const response = await axios.post(`${process.env.APP_URL}/api/refresh-token`, {
        token: oldToken
      });
  ...
  const fetchRecipes = useCallback(async () => {
    try {
      ...
      const response = await axios.get(`${process.env.APP_URL}/api/recipes`, {
  ...
  const fetchNotifications = useCallback(async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await axios.get(`${process.env.APP_URL}/api/notifications`, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleLike = async (recipeId) => {
    try {
      const token = localStorage.getItem('token');
      await axios.post(`${process.env.APP_URL}/api/recipes/${recipeId}/like`, {}, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleNotificationClick = async (notification) => {
    try {
      const token = localStorage.getItem('token');
      await axios.put(
        `${process.env.APP_URL}/api/notifications/${notification.id}/read`,
        {},
        {
          headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleMarkAllRead = async () => {
    try {
      const token = localStorage.getItem('token');
      await axios.put(
        `${process.env.APP_URL}/api/notifications/mark-all-read`,
        {},
        {
          headers: { Authorization: `Bearer ${token}` }
        }
