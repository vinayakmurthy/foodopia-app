...
function RecipeDetails({ 
  ...
  const fetchComments = useCallback(async () => {
    try {
      ...
      const response = await axios.get(`${window.location.origin}/api/recipes/${recipe.id}/comments`, {
  ...
  const checkLikeStatus = useCallback(async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await axios.get(`${window.location.origin}/api/recipes/${recipe.id}/like-status`, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const checkSaveStatus = useCallback(async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await axios.get(`${window.location.origin}/api/recipes/${recipe.id}/save-status`, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleLike = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await axios.post(`${window.location.origin}/api/recipes/${recipe.id}/like`, {}, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleCommentSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(
        `${window.location.origin}/api/recipes/${recipe.id}/comments`,
        { content: newComment },
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  const handleSave = async () => {
    try {
      ...
      const token = localStorage.getItem('token');
      const response = await axios.post(
        `${window.location.origin}/api/recipes/${recipe.id}/save`,
        {},
        {
          headers: { 
  ...
  const preloadAudio = async (text, languageCode) => {
    try {
      ...
      const response = await axios({
        method: 'post',
        url: `${window.location.origin}/api/text-to-speech`,
